...
const FAQS = [
  ...
  },
  {
    ...
    a: "기본은 지금 여신 브라우저(그 기기) 안이에요. 「같이 쓰기」 탭에서 연결하면 무료 Supabase 서버에 실시간으로 공유되고, 기기를 옮길 때는 저녁 이력 탭의 「전체 백업」 파일 하나로도 옮길 수 있어요.",
  },
  {
  ...
  {
    q: "둘이서 동시에 쓸 수 있나요?",
    a: "네! 「같이 쓰기」 탭에서 5분만 설정하면(무료 Supabase) 두 폰이 같은 기록을 실시간으로 공유해요. 와이프가 저녁을 고르면 1초 만에 내 화면에 뜹니다. 설정 없이 쓰실 땐 「전체 백업 / 백업 불러오기」로 메뉴판을 옮기면 돼요.",
  },
  {
    q: "와이프도 Supabase 가입하고 프로젝트를 만들어야 하나요?",
    a: "아니요, 한 번만 만들면 돼요! 프로젝트(=공유 냉장고)는 한 개, 방 코드(=냉장고 열쇠)도 한 개 — 두 사람이 그 한 개를 같이 쓰는 구조예요. 심지어 이 사이트처럼 「내장 연결」이 돼 있으면 가입·설정 없이 방 코드만으로 누구나 연결돼요. 셋이 쓸 일이 생겨도 같은 방 코드에 연결만 하면 돼요.",
  },
  {
    q: "다른 커플(친구들)도 이 앱을 같이 쓰고 싶대요. 어떻게 해요?",
    a: "이 사이트처럼 「내장 연결」이 켜져 있다면, 각 커플은 같이 쓰기 탭에서 자기들 방 코드를 새로 만들고 초대 링크만 주고받으면 끝 — 가입도 설정도 없어요. 방 코드마다 완전히 따로 노는 공간이라 커플끼리 서로 보이지 않아요. 내장 연결이 없는 배포판이라면 커플마다 한 명만 대표로 Supabase에 무료 가입해 자기들 프로젝트를 만들면 됩니다. 한마디로 「한 커플 = 한 방 코드」예요.",
  },
];
...
const TAB_GUIDE = [
  ...
  },
  {
    ...
    tone: "bg-tomato text-card border-tomato-deep",
    title: "같이 쓰기",
    desc: "무료 Supabase로 두 기기를 실시간 연결해요. 한 번 설정하면 와이프가 고른 저녁이 1초 만에 이 화면에 떠요. 방 코드가 비밀번호 역할!",
  },
];
