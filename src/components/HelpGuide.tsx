...
const FAQS = [
  ...
  },
  {
    ...
    a: "아니요, 한 번만 만들면 돼요! 프로젝트(=공유 냉장고)는 한 개, 방 코드(=냉장고 열쇠)도 한 개 — 두 사람이 그 한 개를 같이 쓰는 구조예요. 와이프분은 Supabase 가입 자체가 필요 없고, 초대 링크를 열고 URL이랑 anon 키만 붙여넣으면 끝이에요. 셋이 쓸 일이 생겨도 같은 방 코드에 연결만 하면 돼요.",
  },
  {
    q: "다른 커플(친구들)도 이 앱을 같이 쓰고 싶대요. 어떻게 해요?",
    a: "커플(그룹)마다 한 명만 대표로 Supabase에 무료 가입해서 자기들만의 프로젝트를 만들고, 방 코드를 하나 새로 열면 돼요. 나머지는 그 사람이 보낸 초대 링크만 열면 자동으로 연결됩니다. 방 코드마다 완전히 따로 노는 공간이라, 우리 커플의 메뉴·기록은 다른 커플에게 절대 보이지 않아요. 한마디로 「한 커플 = 한 프로젝트 + 한 방 코드」예요.",
  },
];
